...
namespace JMIR.MAM
{
  public class MAM
  {
    ...
    private readonly MamChannel _channel;
    ...
    public MAM()
    {
      ...
      _channel = this.ChannelFactory.Create(Mode.Restricted, seed, SecurityLevel.Medium, channelKey);
    }
    ...
    public async Task<TimeObj> PublishMessage(string text)
    {
      ...
      var message = _channel.CreateMessage(TryteString.FromUtf8String(text));
      timer.Stop();  
      ...
      await _channel.PublishAsync(message);
      timer.Stop();  
